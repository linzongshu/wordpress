...
function get_calendar($initial = true, $echo = true) {
     ...
     */
	global $wpdb, $m, $monthnum, $year, $wp_locale, $posts, $pi_site_user;

	$cache = array();
//	$key = md5( $m . $monthnum . $year );
    $key = md5( $pi_site_user->ID . '|' . $m . $monthnum . $year );
...
//		$gotsome = $wpdb->get_var("SELECT 1 as test FROM $wpdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 1");
        $gotsome = $wpdb->get_var("SELECT 1 as test FROM $wpdb->posts WHERE post_type = 'post' AND post_status = 'publish' AND post_author = '{$pi_site_user->ID}' LIMIT 1");
		if ( !$gotsome ) {
			$cache[ $key ] = '';
...
//			LIMIT 1");
    $previous = $wpdb->get_row("SELECT MONTH(post_date) AS month, YEAR(post_date) AS year
		FROM $wpdb->posts
		WHERE post_date < '$thisyear-$thismonth-01'
		AND post_type = 'post' AND post_status = 'publish' AND post_author = '{$pi_site_user->ID}'
			ORDER BY post_date DESC
			LIMIT 1");
