...
//			$_bookmark = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->links WHERE link_id = %d LIMIT 1", $bookmark));
            if ($pi_site_user) {
                $_bookmark = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->links WHERE link_id = %d AND link_owner = %d LIMIT 1", $bookmark, $pi_site_user->id));
            } else {
                $_bookmark = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->links WHERE link_id = %d LIMIT 1", $bookmark));
            }
			if ( $_bookmark ) {
...
function get_bookmarks($args = '') {
    ...
    $owner_query = '';
    if ($owner) {
        $owner_query = "AND link_owner = '{$owner}";
    }

	$query = "SELECT * $length $recently_updated_test $get_updated FROM $wpdb->links $join WHERE 1=1 $visible $category_query";
//	$query .= " $exclusions $inclusions $search";
    $query .= " $exclusions $inclusions $search $owner_query";
    /**#@-*/
