...
function get_term_by($field, $value, $taxonomy, $output = OBJECT, $filter = 'raw') {
    /**#@+
     * Hacked for Pi to support to user's term
     */
//	global $wpdb;
	global $wpdb, $pi_site_user;
...
//    $term = $wpdb->get_row( $wpdb->prepare( "SELECT t.*, tt.* FROM $wpdb->terms AS t INNER JOIN $wpdb->term_taxonomy AS tt ON t.term_id = tt.term_id WHERE tt.taxonomy = %s AND $field = %s LIMIT 1", $taxonomy, $value) );
    $current_user = wp_get_current_user() ?: $pi_site_user;
    if ($current_user) {
        $term = $wpdb->get_row( $wpdb->prepare( "SELECT t.*, tt.* FROM $wpdb->terms AS t INNER JOIN $wpdb->term_taxonomy AS tt ON t.term_id = tt.term_id WHERE tt.taxonomy = %s AND $field = %s AND t.term_owner = %d LIMIT 1", $taxonomy, $value, $current_user->ID) );
    } else {
	    $term = $wpdb->get_row( $wpdb->prepare( "SELECT t.*, tt.* FROM $wpdb->terms AS t INNER JOIN $wpdb->term_taxonomy AS tt ON t.term_id = tt.term_id WHERE tt.taxonomy = %s AND $field = %s LIMIT 1", $taxonomy, $value) );
    }
    /**#@-*/
...
function get_terms($taxonomies, $args = '') {
     ...
     * Hacked for Pi to support to user's term
     */
    if (is_wp_admin() && $current_user = wp_get_current_user()) {
        $args['term_owner'] = $current_user->ID;
    }
	$args = wp_parse_args( $args, $defaults );
    ...
    if (!empty($term_owner)) {
        $term_owner = (int) $term_owner;
        $where .= " AND t.term_owner = '$term_owner'";
    }
    /**#@-*/
...
function term_exists($term, $taxonomy = '', $parent = 0) {
	...
	if ( is_int($term) ) {
		if ( 0 == $term )
			return 0;
//		$where = 't.term_id = %d';
        $where = 't.term_id = %d AND t.term_owner = %d';
		if ( !empty($taxonomy) )
//			return $wpdb->get_row( $wpdb->prepare( $tax_select . $where . " AND tt.taxonomy = %s", $term, $taxonomy ), ARRAY_A );
            return $wpdb->get_row( $wpdb->prepare( $tax_select . $where . " AND tt.taxonomy = %s", $term, $current_user->ID, $taxonomy ), ARRAY_A );
		else
//			return $wpdb->get_var( $wpdb->prepare( $select . $where, $term ) );
            return $wpdb->get_var( $wpdb->prepare( $select . $where, $term, $current_user->ID ) );
	}
...
//	$where = 't.slug = %s';
    $where = 't.slug = %s AND t.term_owner = %d';
//	$else_where = 't.name = %s';
    $else_where = 't.name = %s AND t.term_owner = %d';
//	$where_fields = array($slug);
    $where_fields = array($slug, $current_user->ID);
//	$else_where_fields = array($term);
    $else_where_fields = array($term, $current_user->ID);
    /**#@-*/
	if ( !empty($taxonomy) ) {
		$parent = (int) $parent;
		if ( $parent > 0 ) {
...
//		$defaults['default'] = get_option( 'default_category' );
        $defaults['default'] = get_the_author_meta('default_category', $current_user->ID);
...
//	$defaults = array( 'alias_of' => '', 'description' => '', 'parent' => 0, 'slug' => '');
    $defaults = array( 'alias_of' => '', 'description' => '', 'parent' => 0, 'slug' => '', 'term_owner' => $current_user->ID);
	$args = wp_parse_args($args, $defaults);
	$args['name'] = $term;
...
//		$alias = $wpdb->get_row( $wpdb->prepare( "SELECT term_id, term_group FROM $wpdb->terms WHERE slug = %s", $alias_of) );
        $alias = $wpdb->get_row( $wpdb->prepare( "SELECT term_id, term_group FROM $wpdb->terms WHERE slug = %s AND term_owner = %d", $alias_of, $term_owner) );
		if ( $alias->term_group ) {
...
//				if ( false === $wpdb->insert( $wpdb->terms, compact( 'name', 'slug', 'term_group' ) ) )
                if ( false === $wpdb->insert( $wpdb->terms, compact( 'name', 'slug', 'term_group', 'term_owner' ) ) )
...
//		$wpdb->update( $wpdb->terms, compact( 'slug' ), compact( 'term_id' ) );
        $wpdb->update( $wpdb->terms, compact( 'slug' ), compact( 'term_id', 'term_owner' ) );
...
//		$query = $wpdb->prepare( "SELECT slug FROM $wpdb->terms WHERE slug = %s AND term_id != %d", $slug, $args['term_id'] );
        $query = $wpdb->prepare( "SELECT slug FROM $wpdb->terms WHERE slug = %s AND term_owner = %d AND term_id != %d",
            $slug, $current_user->ID, $args['term_id'] );
	else
//		$query = $wpdb->prepare( "SELECT slug FROM $wpdb->terms WHERE slug = %s", $slug );
        $query = $wpdb->prepare( "SELECT slug FROM $wpdb->terms WHERE slug = %s AND term_owner = %d", $slug, $current_user->ID );
...
//			$slug_check = $wpdb->get_var( $wpdb->prepare( "SELECT slug FROM $wpdb->terms WHERE slug = %s", $alt_slug ) );
            $slug_check = $wpdb->get_var( $wpdb->prepare( "SELECT slug FROM $wpdb->terms WHERE slug = %s AND term_owner = %d", $alt_slug, $current_user->ID ) );
		} while ( $slug_check );
		$slug = $alt_slug;
	}
    /**#@-*/
...
function wp_update_term( $term_id, $taxonomy, $args = array() ) {
     ...
     */
    $current_user = wp_get_current_user();
    // @todo Administrator and Editor can update all tags
    // Now only author can edit tags by himself
    if ($current_user->ID != $term->term_owner) {
        $error = new WP_Error('invalid_term', __('Invalid Term'));
        return $error;
    }
	$defaults = array( 'alias_of' => '', 'description' => '', 'parent' => 0, 'slug' => '');
	$args = wp_parse_args($args, $defaults);
...
//	$id = $wpdb->get_var( $wpdb->prepare( "SELECT term_id FROM $wpdb->terms WHERE slug = %s", $slug ) );
    $id = $wpdb->get_var( $wpdb->prepare( "SELECT term_id FROM $wpdb->terms WHERE slug = %s AND term_owner = %d", $slug, $term_owner ) );
	if ( $id && ($id != $term_id) ) {
